## First in First out (LIFO) Tax Strategy.
...
for x in range(len(transactions)):
    if transactions['transaction_units'][x] < 0:
        ...
        # Returns the index going from zero to x that first has a value for available_to_sell
        counter = next((i for i, count in enumerate(transactions.available_to_sell) if count), None)
